...
url='https://qcloud.coding.net/u/lm93129/p/mysave/git/raw/master'
...
yum install docker-ce-17.09.1.ce -y;
fi
...
if test $os == 6 ;then
echo '******正在下载，请等待几分钟******';
wget ${url}/rpm/metricbeat-6.4.1-x86_64.rpm;
wget ${url}/rpm/filebeat-6.4.1-x86_64.rpm;
echo -e '下载完成：			  [\033[32m  OK  \033[0m]';
echo '******正在安装******';
sudo rpm -vi metricbeat-6.4.1-x86_64.rpm;
sudo rpm -vi filebeat-6.4.1-x86_64.rpm;
echo '性能监控配置文件路径：/etc/metricbeat/modules.d/';
echo '日志配置文件路径：/etc/filebeat/modules.d/';
sh myshell.sh;
fi
...
if test $os == 10 ;then
echo '******下载解压******';
wget ${url}/rpm/pinpoint-agent-1.8.0.tar.gz -P /home/Downloads/pinpoint-agent && tar zxvf /home/Downloads/pinpoint-agent/pinpoint-agent-1.8.0.tar.gz;
fi
